...
export function SocialLinksSection() {
  ...
  return (
    <SectionCard title="Social Links" description="Additional profile and portfolio links">
      {fields.length === 0 ? (
        <p className="text-xs text-gray-500">No social links yet.</p>
      ) : (
            ...
            <SortableItem key={field.id} id={field.id}>
              <div className="space-y-3">
                <div className="flex items-center justify-between gap-2">
                  <p className="text-xs font-bold text-gray-700">Link {index + 1}</p>
                  <Button
                    type="button"
                    variant="outline"
                    size="sm"
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  <FieldRow label="Label">
                    <Input placeholder="Portfolio, Twitter, etc." {...register(`socialLinks.${index}.label`)} />
                  </FieldRow>
                  <FieldRow
                    label="URL"
                    error={socialErrors?.[index]?.url?.message}
                  >
      ...
      <Button type="button" variant="outline" size="sm" onClick={() => append(emptySocialLink)}>
